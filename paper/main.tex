...
% Theorem-like environments (definitions upright, statements italic)
...
% Running heads: short title / "Preprint", plain first page
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\itshape Explainability is Relational}
...
% Typographic quality: no widows/orphans, forgiving line breaking
\clubpenalty=10000
\widowpenalty=10000
\displaywidowpenalty=10000
\emergencystretch=1.5em

% Compact, consistent heading spacing
\usepackage{titlesec}
\titlespacing*{\section}{0pt}{14pt plus 3pt minus 2pt}{7pt plus 1pt}
\titlespacing*{\subsection}{0pt}{11pt plus 2pt minus 2pt}{5pt plus 1pt}
\titlespacing*{\paragraph}{0pt}{7pt plus 2pt minus 1pt}{0.9em}
...
This paper is a position paper with preliminary evidence.

\paragraph{Contributions.}
\begin{enumerate}[label=(\roman*),leftmargin=2.4em]
  \item \textbf{A formal framework} in which explainability is defined relative to a receiver $R$ and a contextual question $Q$, with two operational measures --- the explainability curve and the number of why-queries --- and classical XAI recovered as the aligned degenerate case (Section~\ref{sec:framework}).
  \item \textbf{Controlled evidence.} On a synthetic world whose explanatory ground truth is known by construction, the cost of explanation is governed by the alignment between the bases of $M$ and $R$, not by $M$ alone: it grows monotonically with basis misalignment (Section~\ref{sec:exp1}), is invariant to monotone reparametrizations (Section~\ref{sec:exp1b}), reappears identically in an interactive protocol (Section~\ref{sec:exp2}), and depends asymmetrically on the question (Section~\ref{sec:exp4}).
  \item \textbf{The relation meets reality.} On real data the framework predicts \emph{where} natural misalignment appears; we confirm both the negative and the positive prediction with pre-registered criteria, and find the effect modulated by the model's own internal basis (Section~\ref{sec:real}).
  \item \textbf{The inverse channel.} Because explanation is a channel, it can be used in reverse: editing an explainable graph through dialogue outperforms gradient-based adaptation under drift, with zero cumulative forgetting by construction; we state the corresponding conjecture explicitly (Section~\ref{sec:interactive}).
\end{enumerate}
...
\begin{definition}[Receiver, candidate explanation]
\label{def:receiver}
A \emph{receiver} is a pair $R = (V_R, C_R)$: a vocabulary of predicates $V_R$ --- the tests $R$ can evaluate and understand --- and a capacity constraint $C_R$ bounding the size of the structures $R$ can hold. A \emph{candidate explanation} of a model $M$ for $R$ is a decision graph $G$ whose internal nodes are predicates of $V_R$, whose edges carry the conditional flow, and whose leaves are outputs.
\end{definition}

\begin{definition}[Relational explainability]
\label{def:expl}
Let $\mathrm{fid}(G, M)$ be the agreement between $G$ and $M$ on the data distribution. The explainability of $M$ for $R$ at fidelity level $\varepsilon$ is the size of the smallest faithful explanation,
\begin{equation}
\label{eq:expl}
\mathrm{Expl}(M, R, \varepsilon) \;=\; \min \bigl\{\, |G| \;:\; G \text{ expressed in } V_R,\ \ \mathrm{fid}(G, M) \ge 1 - \varepsilon \,\bigr\},
\end{equation}
and $M$ is \emph{explainable for $R$} at level $(\varepsilon, K)$ if and only if $\mathrm{Expl}(M, R, \varepsilon) \le K$. The \emph{explainability curve} of the pair $(M, R)$ maps each budget $|G|$ to the maximal fidelity attainable within it --- the explainability signature of $M$ \emph{relative to} $R$; its area under the curve (AUC) is the associated scalar metric.
\end{definition}

\paragraph{Degenerate case.} If $V_R$ consists of threshold predicates on $M$'s own input features (the identity basis), Equation~\eqref{eq:expl} reduces to classical tree distillation: standard XAI is relational explainability restricted to perfect alignment.

Two axes of complexity interact in Definition~\ref{def:expl}: structural complexity (the size of the graph) and predicate complexity (the vocabulary $V_R$). A small graph with dense predicates ($w \cdot x > \theta$) is not readable; an axis-aligned graph is. The ``receiver's basis'' is $V_R$.
...
\paragraph{Results (Table~\ref{tab:exp1}, Figure~\ref{fig:exp1}).} The explainability AUC decreases strictly monotonically with $\theta$: $0.918$ at $\theta{=}0$ down to $0.745$ at $\theta{=}1.5$. The minimal budget for fidelity $\ge 0.95$ is 16 leaves at $\theta{=}0$, 128 at $\theta{=}0.2$, and is never reached (${>}256$) for $\theta \ge 0.4$.

\paragraph{Interpretation.} Same model $M$ (not one weight changes), same information (rotations are bijections). Only the receiver's basis varies, and the explainability curves diverge monotonically with $\theta$. Explainability is therefore not a property of $M$ alone.

\subsection{Experiment 1b: what costs is mixing, not reparametrization}
\label{sec:exp1b}

Applying a coordinate-wise monotone deformation on top of the rotation ($v = \tanh(s \cdot (x - 0.5) \cdot R_{\theta=1.5})$, $s \in \{2, 6\}$, 5 seeds) leaves the AUC unchanged: $0.7432$ vs.\ $0.7433$ vs.\ $0.7433$ for the linear, $s{=}2$, and $s{=}6$ cases.

\begin{proposition}[Monotone invariance; empirical]
\label{prop:monotone}
For a receiver whose vocabulary consists of threshold predicates, the cost of explainability is invariant under coordinate-wise monotone deformations of its basis.
\end{proposition}

The intuition is immediate --- a threshold transports through any monotone function, so threshold trees depend only on coordinate orders --- but we state Proposition~\ref{prop:monotone} as an empirical claim: it was confirmed on every test bed of this paper, synthetic and real, 11 runs out of 11, often at exactly $0.0000$ (Sections~\ref{sec:exp5} and~\ref{sec:exp6}); the formal proof is left to future work. Its consequence sharpens the concept: what costs is the \emph{mixing} of dimensions, not their reparametrization --- the relevant misalignment is structural, not metric.

\subsection{Experiment 2: explanation as a measurable protocol}
\label{sec:exp2}

\paragraph{Protocol.} Same world, same $M$; the reference graph of $M$ is a 32-leaf tree distilled in the original basis. The operator $W(x)$ answers ``why is $x$ classified $c$?'' with the decision path of $x$ (a conjunction of threshold predicates) plus its class --- minimality via the path, implicit contrast versus the other leaves. Dialogue protocol (active learning): the receiver starts from a trivial majority-class predictor; at each turn it picks a point where it disagrees with $M$, asks $W$, integrates the rule, and repeats (15 questions max, 5 seeds). The \emph{aligned} receiver stores the rule verbatim; the \emph{rotated} receiver ($\theta{=}1.5$) perceives the world only in its own basis, so each received rule --- an axis-aligned region for $M$, an oblique polytope for it --- must be approximated by a small 8-leaf tree in \emph{its} vocabulary.
...
\paragraph{Results (Table~\ref{tab:exp2}, Figure~\ref{fig:exp2}).} The aligned receiver reaches $90\%$ fidelity in 6 questions and $97.9\%$ at 15; the rotated receiver never reaches $90\%$ and plateaus at ${\sim}71\%$. Structurally, $G^\ast$ has ${\sim}7$ branches and ${\sim}6$ questions suffice: the dialogue reconstructs the structure at roughly one rule per question.

\paragraph{Interpretation.} Explainability-as-protocol is measurable: the number of why-questions is small and finite when bases are aligned, and the dialogue fails to converge when they are misaligned --- \emph{with identical answers from $M$}. Two independent protocols (static distillation, interactive dialogue) confirm the same thesis.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\linewidth]{exp7_living_graph.png}
...
\paragraph{Results (Table~\ref{tab:exp7}, Figure~\ref{fig:exp7}).} Fifteen sentences with zero labels outperform $75{,}000$ labels of from-scratch retraining ($0.990 \pm 0.003$ vs.\ $0.981 \pm 0.002$). All four kill conditions survived. Integrity of the never-edited region after 15 edits: $0.992 \to 0.994$ --- zero cumulative degradation (K2: locality holds over 15 edits). Human imprecision of $\pm 0.10$ costs only ${\sim}2$ points ($0.971$). The misaligned receiver pays a measured \emph{expressibility tax} of ${\sim}0.19$ ($0.801$), quantitatively linking Experiments 1--2 to Experiment 3. An unplanned control: the cyclic schedule returns the world to its initial state at drift 12, where the frozen model climbs back exactly to its original level --- the drift mechanics are sound.

\begin{conjecture}[Post-gradient learning]
\label{conj:postgradient}
Learning by graph editing in dialogue is a candidate paradigm for post-gradient learning: data-efficient because the channel is semantic, and continual by construction because edits are local.
\end{conjecture}

Conjecture~\ref{conj:postgradient} attacks head-on the two best-documented weaknesses of deep learning: data inefficiency (one sentence compresses thousands of examples, Experiment 3) and catastrophic forgetting (locality removes the problem rather than mitigating it, Experiment 7, condition K2). We state it as a conjecture deliberately: it is supported by controlled synthetic evidence, not an established claim.
